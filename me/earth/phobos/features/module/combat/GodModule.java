...
public class GodModule extends Module {
   ...
   public void onSendPacket(PacketEvent.Send event) {
      if (event.getStage() == 0 && event.getPacket() instanceof CPacketPlayerTryUseItemOnBlock) {
         ...
         if (mc.field_71439_g.func_184586_b(packet.field_187027_c).func_77973_b() instanceof ItemEndCrystal) {
            if ((Boolean)this.checkPos.getValue() && !BlockUtil.canPlaceCrystal(packet.field_179725_b, (Boolean)this.entitycheck.getValue(), (Boolean)this.oneDot15.getValue()) || this.checkPlayers()) {
               return;
            }

            this.updateEntityID();

            for(int i = 1; i < (Integer)this.attacks.getValue(); ++i) {
   ...
   public void rotateTo(BlockPos pos) {
      float[] angle = MathUtil.calcAngle(mc.field_71439_g.func_174824_e(mc.func_184121_ak()), new Vec3d(pos));
      this.yaw = angle[0];
      this.pitch = angle[1];
      this.rotating = true;
   }
   ...
   public static class AttackThread extends Thread {
      ...
      public void run() {
         try {
            ...
            attack.field_149566_b = Action.ATTACK;
            this.godModule.rotateTo(this.pos.func_177984_a());
            Util.mc.field_71439_g.field_71174_a.func_147297_a(attack);
