...
public class ModuleManager extends Feature {
   ...
   public ArrayList<Module> getEnabledModules() {
      ...
      while(true) {
         ...
         do {
            ...
            module = (Module)var2.next();
         } while(!module.isEnabled() && !module.isSliding());
   ...
   public void alphabeticallySortModules() {
      this.alphabeticallySortedModules = (List)this.getEnabledModules().stream().filter(Module::isDrawn).sorted(Comparator.comparing(Module::getDisplayName)).collect(Collectors.toList());
   }
   ...
   public void onKeyPressed(int eventKey) {
      if (eventKey != 0 && Keyboard.getEventKeyState() && !(mc.field_71462_r instanceof PhobosGui)) {
         this.modules.forEach((module) -> {
   ...
   public List<Module> getAnimationModules(Module.Category category) {
      List<Module> animationModules = new ArrayList();
      Iterator var3 = this.getEnabledModules().iterator();
